...
{
    public partial class doctorLabs : UserControl
    {
        ...
        private void view()
        {
            var lab = from r in clinic.organizations
                      where r.tybe == "lab"
        ...
        private void search_Click(object sender, EventArgs e)
        {
            ...
            else if (searchBox2.Text == "إسم المعمل")
            {
                var lab = from l in clinic.organizations
                          where l.tybe == "lab" && l.org_name.Contains(searchcontent2.Text)
            ...
            {
                var lab = from l in clinic.organizations
                          where l.tybe == "lab" && l.phoneNUMBER.Contains(searchcontent2.Text)
                          let الإسم = l.org_name
                          let رقم_الهاتف = l.phoneNUMBER
                          let العنوان = l.address
                          select new { l.Oid, الإسم, رقم_الهاتف, العنوان };
            ...
            {
                var lab = from l in clinic.organizations
                          where l.tybe == "lab" && l.address.Contains(searchcontent2.Text)
                          let الإسم = l.org_name
                          let رقم_الهاتف = l.phoneNUMBER
                          let العنوان = l.address
                          select new { l.Oid, الإسم, رقم_الهاتف, العنوان };
